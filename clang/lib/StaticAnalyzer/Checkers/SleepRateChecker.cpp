//==- SleepRateChecker.cpp - Check for stores to dead variables -*- C++ -*-==//
//
...
//===----------------------------------------------------------------------===//
//
//  This file defines a SleepRate, a flow-sensitive checker that looks for
...
namespace {
class SleepRateChecker : public Checker<check::ASTCodeBody> {
public:
...
class DeadStoreObs : public LiveVariables::Observer {
  ...
  ASTContext &Ctx;
  BugReporter& BR;
  const SleepRateChecker *Checker;
...
public:
  DeadStoreObs(const CFG &cfg, ASTContext &ctx, BugReporter &br,
               const SleepRateChecker *checker, AnalysisDeclContext *ac,
               ParentMap &parents,
               llvm::SmallPtrSet<const VarDecl *, 20> &escaped,
...
void SleepRateChecker::checkASTCodeBody(const Decl *D, AnalysisManager &mgr,
                                         BugReporter &BR) const {
...
void ento::registerSleepRateChecker(CheckerManager &Mgr) {
  auto *Chk = Mgr.registerChecker<SleepRateChecker>();

  const AnalyzerOptions &AnOpts = Mgr.getAnalyzerOptions();
  Chk->WarnForDeadNestedAssignments =
      AnOpts.getCheckerBooleanOption(Chk, "FindSleepRates");
  Chk->ShowFixIts =
      AnOpts.getCheckerBooleanOption(Chk, "ShowFixIts");
}

bool ento::shouldRegisterSleepRateChecker(const CheckerManager &mgr) {
  return true;
}
